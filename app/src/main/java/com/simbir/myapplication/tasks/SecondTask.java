...
public class SecondTask {
    ...
    at the 1st position of the array, the sum of left side of the index ({1}) and the sum of
    ...
    */
    ...
    public static int findEvenIndex(int[] arr) {
        ...
        for(int i = 0; i < arr.length; i++){
            if(i == 0){
                int leftSide = 0;
                int rightSide = summation(Arrays.copyOfRange(arr, i + 1, arr.length));
                if(leftSide == rightSide) {
                    result = i;
                }
            }else{
                int leftSide = summation(Arrays.copyOfRange(arr, 0, i));
                int rightSide = summation(Arrays.copyOfRange(arr, i + 1, arr.length));
                if(leftSide == rightSide){
                    if(result == -1){
                        result = i;
    ...
    private static int summation(int[] arrayForSum){
        int sum = 0;
